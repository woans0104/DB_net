...
import numpy as np
#from shapely.geometry import Polygon as Polygon3
from shapely.geometry import Point


from structure.measurers.clEval.rrc_evaluation_funcs import parse_single_file, decode_utf8, main_evaluation
from structure.measurers.clEval.file_utils import load_zip_file, load_dir_file, decode_utf8, make_zip
from structure.measurers.clEval.validation import validate_data
...
def eval_single_result(gt_file, det_file):
    ...
    gt_boxes = parse_single_file(gt_file, PARAMS.CRLF, PARAMS.BOX_TYPE, True, False)
    sample_result.prepare_gt(gt_boxes)
    det_boxes = parse_single_file(det_file, PARAMS.CRLF, PARAMS.BOX_TYPE,
...
def cleval_evaluation(gt_file, submit_file):
    ...
    if extension == '.zip':
        ...
        gt_files = load_dir_file(gt_file,'label')
    ...
    for file_idx in gt_files:
        if type(gt_files[file_idx]) is bytes:
            gt_file = decode_utf8(gt_files[file_idx])
...
def main(gt_dir, pred_dir, GT_BOX_TYPE='QUAD', PRED_BOX_TYPE='QUAD', TRANSCRIPTION=True, CONFIDENCES=False, CRLF=False):
    ...
    # evaluate dectection
    result = main_evaluation(validate_data, cleval_evaluation, show_result=False)
    ...
    try:
        ...
        print("Error: %s : %s" % (gt_dir, e.strerror))



    final_dic = {}

    final_dic['precision'] = pr
    final_dic['recall'] = re
...
if __name__ == '__main__':
    ...
    main_evaluation(validate_data, cleval_evaluation)

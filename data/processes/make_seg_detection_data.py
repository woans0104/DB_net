...
class MakeSegDetectionData(DataProcess):
    ...
    def process(self, data):
        ...
        # cv2.imwrite('gt.png', gt[0]*255)
        # cv2.imwrite('image.png', image)
        #
        # gt_t = np.invert(gt[0].astype(bool)).astype(int)
        # gt_t = np.expand_dims(gt_t, axis=-1)
        # gt_t_ = np.concatenate([gt_t,gt_t,gt_t],axis=-1)
        #
        # cv2.imwrite('overay_image.png', image*gt_t_)
        # import ipdb;
        # ipdb.set_trace()
        # ------------------------------------------------------------------------------#

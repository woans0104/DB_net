...
class Trainer:
    ...
    def validate(self, validation_loaders, model, epoch, step):
        ...
        for name, loader in validation_loaders.items():
            ...
            if self.experiment.validation.visualize:
                metrics, vis_images = self.validate_step_cleval(
                    loader, model, True)
                ...
                metrics, vis_images = self.validate_step_cleval(loader, model, False)
    ...
    #------------------------------------------------------------#
    # cleval validation
    def validate_step_cleval(self, data_loader, model, visualize=False):
        ...
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # cleval validation
            self.structure.measurer.validate_measure(
                batch, output, dir_path=self.model_saver.dir_path)
